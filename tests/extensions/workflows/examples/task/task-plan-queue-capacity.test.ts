...
function capacityRun(repaired: boolean) {
  const calls: Call[] = [];
  ...
  const lateRepair = repaired ? ["review route + single correction", "recheck route"] : lateProposal;
  const queues: Record<string, unknown[]> = {
    ...
    "workflow-source-queue-recheck": [
      ...early.map(() => "Queue valid"),
      repaired ? "All identities retained in two edits" : "Capacity conflict remains",
      ...(repaired ? ["Queue valid", "Whole graph complete"] : []),
    ],

...
export default async function runWorkflow(dsl, input = "") {
  ...
  for (let accepted = 0; accepted <= 6; accepted += 1) {
    ...
    const proposedRoute = await dsl.agent(
      `Translate the queue assessment without rejudging it. Choose work for a complete conflict-free non-empty queue. Choose complete only when the actual whole file satisfies every design requirement and the proposed queue is empty. Choose queue_conflict when an unmet source identity disappeared (including from a grouped item), an accepted identity returned, the queue contradicts the file or reviewed graph, a branch omits a required choice destination or terminal behavior, safe grouping cannot fit the remaining source-slice allowance, or completion conflicts with remaining work. An empty prior queue on the first pass is expected and is not a conflict.\n\n${proposedAssessment}`,
      {

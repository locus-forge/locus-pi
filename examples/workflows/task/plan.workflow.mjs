...
export default async function runWorkflow(dsl, input = "") {
  ...
  if (seedRoute !== "passed") {
    const seedFix = await dsl.agent(
      `Use the failed seed check to repair exactly its defects in the existing workspace workflow.mjs once. If the file is absent, do not create it here; report that seed creation failed. Otherwise restore standard orchestration-only grammar, the primary output and scope, and a runnable noncontradictory starting route. If the primary route is missing, add an agent explicitly directed to produce the reviewed primary output in scope; a clarification-only route or diagnostic mention is insufficient. Pass semantic input whole inside an agent prompt, with a default only at the runWorkflow parameter boundary; do not use input || a fallback in the body. A partial adaptive route may remain only if it fails closed instead of claiming final success; list its missing graph nodes for later source slices. Preserve source outside the failed criteria. Write workflow-source-seed-fix.md with path, exact edit outcome and remaining source work. Return only report and paths, never source bytes. ${SOURCE_EDIT_PREFLIGHT}\n\n${SOURCE_CONTRACT}\n\nReviewed design:\n${reviewedDesign}\n\nFailed seed check:\n${seedCheck}`,
